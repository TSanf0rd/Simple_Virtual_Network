...
custom_test_1_basic() {
    # This test is technically valid, though easily hackable and bypassable...
    # It assumes you ran your code at least once on a local Fedora machine,
    # and committed/pushed afterwards.
    # Please don't touch this manually ;)
    if grep --color=auto "Fedora" /etc/os-release && ! grep 'docker\|lxc' /proc/1/cgroup; then
        touch canttouchthis
        git add canttouchthis
    fi
    if [ -f canttouchthis ]; then
        custom_test_score=100
    else
        echo "You're not running the required VM..."
        custom_test_score=0
    fi
}
######## <- Custom tests ########
